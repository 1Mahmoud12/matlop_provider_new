...
class AppVersionChecker {
  ...
  Future<AppCheckerResult> _checkPlayStore(
    ...
    final uri = Uri.https(
      'play.google.com',
      '/store/apps/details',
      {'id': packageName, 'timestamp': '${DateTime.now().millisecondsSinceEpoch}'},
    );
    try {
      final response = await http.get(uri);
      if (response.statusCode != 200) {
        ...
        if (match != null) {
          ...
          final altMatch1 = RegExp(r'"version":"([0-9,\.]*)"').firstMatch(response.body);
          final altMatch2 = RegExp(r'\[\[\["([0-9,\.]*)"]],').firstMatch(response.body);
          final altMatch3 = RegExp(r'Current Version</div><span class="htlgb"><div class="IQ1z0d"><span class="htlgb">([0-9,\.]*)</span>')
